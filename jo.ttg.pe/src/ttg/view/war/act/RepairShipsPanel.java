...
public class RepairShipsPanel extends HelpPanel implements ListSelectionListener, PropertyChangeListener
{
	...
	{
		mPicked = 0;
		ArrayList ships = (ArrayList)mPanel.getArg1();
		...
		mShips.setListData(ships.toArray());
		mWorld.setObject(mWorldInst);
		int port = mWorldInst.getWorld().getPopulatedStats().getUPP().getPort().getValue();
		if (port == 'A')
		{
