...
public class MoveHandler extends BaseHandler
{
	...
	private ArrayList	mScoutShips;
	private ArrayList	mMovingShips;
	private ArrayList	mStationaryShips;
	private HashMap		mLastKnownDefenseFactors;
	private HashMap		mCanDeploy;
	private HashSet		mScoutedWorlds;
	private HashMap		mUnderdefended;
	private HashMap		mUnderattacked;
	private ArrayList	mLongRangePlanner;
	...
	public void move()
	{
		...
		int todo = mMovingShips.size() + mUnderattacked.size() + mUnderdefended.size();
		while (mMovingShips.size() > 0)
		{
			allocateUnderdefended(); 
			allocateUnderattacked();
			int newTodo = mMovingShips.size() + mUnderattacked.size() + mUnderdefended.size();
			if (todo == newTodo)
			{
	...
	private void findArena()
	{
		...
		for (Iterator i = getSide().getShips().iterator(); i.hasNext(); )
		{
			...
			ArrayList worlds = ShipLogic.validExtendedDestinations(getGame(), ship);
			if (!getGame().getGame().isAllowConvertNeutral())
				for (Iterator j = worlds.iterator(); j.hasNext(); )
				{
					WorldInst world = (WorldInst)j.next();
		...
		for (Iterator i = homeland.iterator(); i.hasNext(); )
		{
			WorldInst world = (WorldInst)i.next();
		...
		{
			mHomelandValue[i] = worldValue(mHomeland[i]);//WorldLogic.getHaveWorldValue(getGame(), mHomeland[i]);
		...
		{
			mBorderValue[i] = WorldLogic.getHaveWorldValue(getGame(), mBorder[i]);
	...
	private void reserveDamaged(WorldInst world, int canRepair)
    {
		...
		if (world.getShips().size() == 0)
			return;
        ArrayList wounded = new ArrayList();
	...
	private void allocateScouts()
	{
		...
		for (int i = 0; i < mBorder.length; i++)
		{
			...
			if (WorldLogic.isWitness(mBorder[i], getSide()))
				continue;
			ArrayList potentials = (ArrayList)mCanDeploy.get(mBorder[i]);
			for (Iterator j = potentials.iterator(); j.hasNext(); )
			{
				ShipInst potential = (ShipInst)j.next();
	...
	private void allocateOffense()
	{
		...
		for (int i = 0; i < mBorder.length; i++)
		{
			...
			{
				mUnderattacked.put(mBorder[i], new Integer(estimatedDefense(mBorder[i])));
				continue;
			}
			int bringToBear = 0;
			ArrayList locals = new ArrayList();
			ArrayList foreigners = new ArrayList();
			for (Iterator j = potentials.iterator(); j.hasNext(); )
			{
				ShipInst potential = (ShipInst)j.next();
			...
			if (needed <= 0)
				continue;
			mUnderattacked.put(mBorder[i], new Integer(needed)); // should never happen
		}
	...
	private void allocateDefense()
	{
		...
		for (int i = 0; i < mHomeland.length; i++)
		{
			...
			idealDefense = (int)Math.ceil(idealDefense*getParanoia(mHomeland[i]));
			int actualDefense = 0;
			...
			for (Iterator j = mHomeland[i].getShips().iterator(); j.hasNext(); )
			{
			...
			ArrayList potentials = (ArrayList)mCanDeploy.get(mHomeland[i]);
			if ((potentials == null) || (potentials.size() == 0))
				continue;
			...
			mUnderdefended.put(mHomeland[i], new Integer(idealDefense - actualDefense));
		}
	...
	{
		for (Iterator i = mLongRangePlanner.iterator(); i.hasNext(); )
		{
			LongRangePlanner lrp = (LongRangePlanner)i.next();
			if (!mMovingShips.contains(lrp.ship))
			{
				debug(name(lrp.ship)+" redployed from long range plan to "+name(lrp.world)+".");
				i.remove();
				continue;
			}
			if (mUnderattacked.containsKey(lrp.world))
			{
				...
				ShipLogic.setDestination(getGame(), lrp.ship, (WorldInst)lrp.path.get(0));
				lrp.path.remove(0);
				if (lrp.path.size() == 0)
					i.remove();
				...
				if (amnt.intValue() <= att)
					...
					mUnderattacked.put(lrp.world, new Integer(amnt.intValue() - att));
			...
			if (mUnderdefended.containsKey(lrp.world))
			{
				...
				if (amnt.intValue() <= att)
					...
					mUnderdefended.put(lrp.world, new Integer(amnt.intValue() - att));
				mMovingShips.remove(lrp.ship);
				continue;
			}
	...
	{
		for (Iterator i = mUnderattacked.keySet().iterator(); i.hasNext(); )
		{
			WorldInst world = (WorldInst)i.next();
	...
	{
		for (Iterator i = mUnderdefended.keySet().iterator(); i.hasNext(); )
		{
			WorldInst world = (WorldInst)i.next();
	...
	private LongRangePlanner findClosest(ArrayList ships, WorldInst world)
	{
		...
		{
			ShipInst ship = (ShipInst)i.next();
			ArrayList path = PathLogic.findPath(getGame(), ship.getLocation(), world, 
				ship.getShip().getJump(), ship.getFuel()/ShipLogic.fuelForJump1(ship),
				ShipLogic.fuelTankage(ship)/ShipLogic.fuelForJump1(ship));
			if (path.size() <= 1)
			{
				debug("Can't path from "+OrdLogic.getShortNum(ship.getLocation().getOrds())+
	...
	private int whatShouldWeAttackWith(WorldInst world, int bringToBear)
	{
		...
		int minForce = (int)Math.ceil(def*mPlayer.getMinimalAttackThreshold());
		int maxForce = (int)Math.ceil(def*mPlayer.getDesiredAttackThreshold());
	...
	private boolean deployTowards(ShipInst ship, WorldInst world)
	{
		if (ship.getLocation() == world)
			return true;
		ArrayList valid = ShipLogic.validDestinations(getGame(), ship);
		if (valid.contains(ship))
		{
		...
		if (path.size() > 1)
		{
			...
			debug("     deploying "+name(ship)+" via "+name(step)+" towards "+world);
			StringBuffer sb = new StringBuffer("{");
		...
		{
			debug("**** Can't get from "+name(ship.getLocation())+" to "+name(world));
			return false;
		}
	}
	...
	private void addDeployability(ShipInst ship, ArrayList worlds)
	{
		for (Iterator i = worlds.iterator(); i.hasNext(); )
		{
			WorldInst world = (WorldInst)i.next();
			ArrayList ships = (ArrayList)mCanDeploy.get(world);
			if (ships == null)
			{
				ships = new ArrayList();
