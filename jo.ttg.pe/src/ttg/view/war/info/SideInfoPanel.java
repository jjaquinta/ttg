...
public class SideInfoPanel extends HelpPanel
{
	...
	private void initLink()
	{
		mWorlds.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent ev) {
				if (ev.getClickCount() == 2) doWorldAction();			
			}
		});
	}
	...
	public void setObject(SideInst obj)
	{
		...
		else
		{
			...
			double pop = 0;
			int maxTech = 0;
			for (Iterator i = mSide.getWorlds().iterator(); i.hasNext(); )
			{
				WorldInst world = (WorldInst)i.next();
				MainWorldBean mw = world.getWorld();
				if (mw == null)
					continue;
				resGen += WorldLogic.getResourceGeneration(mPanel.getGame(), world);
				pop += mw.getPopulatedStats().getUPP().getPop().getPopulation();
				maxTech = Math.max(maxTech, mw.getPopulatedStats().getUPP().getTech().getValue());
			}
			int att = 0;
			int def = 0;
			int tons = 0;
			for (Iterator i = mSide.getShips().iterator(); i.hasNext(); )
			{
			...
			if (mPanel.getGame().getGame().isAllowOmniscentSensors()
				...
				|| (mPanel.getSide() == mSide))
					...
					mShips.init(new ArrayList());
			mWorlds.setListData(mSide.getWorlds().toArray());
		}
	}
