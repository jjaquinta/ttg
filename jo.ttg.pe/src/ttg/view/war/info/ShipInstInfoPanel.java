...
public class ShipInstInfoPanel extends HelpPanel
{
	...
	public void setObject(ShipInst obj)
	{
		...
		else
		{
			...
			mAttack.setText(mShip.getShip().getAttack()+" ("+(mShip.getShip().getAttack()/2)+")");
			mDefense.setText(mShip.getShip().getDefense()+" ("+(mShip.getShip().getDefense()/2)+")");
			int cap = ShipLogic.capacity(mShip.getShip());
			if (cap > 0)
				mCapacity.setText(String.valueOf(ShipLogic.additionalCapacity(mShip))
					+" remaining of "
			...
			mFuel.setString(String.valueOf(mShip.getFuel()+" of "+ShipLogic.fuelTankage(mShip)));
			mFuel.setMaximum(ShipLogic.fuelTankage(mShip));
			mFuel.setValue(mShip.getFuel());
			mFuelRequired.setText(String.valueOf(ShipLogic.fuelForJump1(mShip)));
