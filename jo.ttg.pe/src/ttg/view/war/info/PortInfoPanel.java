...
public class PortInfoPanel extends HelpPanel
{
	...
	private void initLink()
	{
		...
		mBuilding.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent ev) {
				if (ev.getClickCount() == 2) doActivateShip();			
				else if (ev.getClickCount() == 1) doSelectShip();			
			}
		});
	}
	...
	public void setObject(WorldInst obj)
	{
		...
		if (mWorld == null)
		{
			...
			mBuilding.setListData(new Object[0]);
		...
		else
		{
			...
			if (mWorld.getWorld() != null)
			{
				...
				int constPerTurn = WorldLogic.getConstructionPerTurn(mPanel.getGame(), mWorld);
				mConstRate.setText(String.valueOf(constPerTurn)
					+ " / turn");
				if ((mWorld.getSide() == mPanel.getSide())
					&& mPanel.getGame().getGame().isAllowConstruction())
				{
					setCanBuild(true);
					mBuilding.setListData(mWorld.getUnderConstruction().toArray());
				}
				else
				{
					mBuilding.setListData(new Object[0]);
	...
	private void doSelectShip()
	{
		...
		else
		{
			...
			int constPerTurn = WorldLogic.getConstructionPerTurn(mPanel.getGame(), mWorld);
			int left = (int)(Math.ceil((double)(cost - done)/(double)constPerTurn));
	...
	private void doBuild()
	{
		...
		DlgShipDesigner dlg = new DlgShipDesigner((Frame)SwingUtilities.getRoot(this));
		dlg.setShip(design);
		dlg.setJumpLimit(WorldLogic.getMaxJumpConstruction(mWorld));
		dlg.setConstRate(WorldLogic.getConstructionPerTurn(mPanel.getGame(), mWorld));
	...
	{
		ArrayList ships = mWorld.getUnderConstruction();
		int idx = mBuilding.getSelectedIndex();
