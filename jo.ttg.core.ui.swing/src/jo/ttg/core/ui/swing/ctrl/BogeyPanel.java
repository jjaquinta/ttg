...
public class BogeyPanel extends JComponent
{
	...
	private ArrayList	mBogies;
	private ArrayList	mLinks;
	private HashMap		mBogieLocation;
	private ArrayList	mSelectable;
	...
	private ArrayList	mTTGActionListeners;
	...
	private void initLink()
	{
		...
		addComponentListener(new ComponentAdapter() {public void componentResized(ComponentEvent ev)
        {
	        mScaleUpdated = false;
        }} );
	}
	...
	protected void doProcessMouseEvent(MouseEvent e)
	{
		...
		switch (e.getID())
		{
			case MouseEvent.MOUSE_CLICKED:
				...
				{
					fireTTGActionEvent(new TTGActionEvent(this, TTGActionEvent.SELECTED, "", b));				
				}
				else if (e.getClickCount() == 2)
				{
					fireTTGActionEvent(new TTGActionEvent(this, TTGActionEvent.ACTIVATED, "", b));				
				}
				break;
...
//				break;
			case MouseEvent.MOUSE_MOVED:
				...
				fireTTGActionEvent(new TTGActionEvent(this, TTGActionEvent.HOVER, "", b));
...
//				break;
    ...
    private void updateScale()
    {
	    ...
	    if (xScale < yScale)
	        ...
	        mScale = yScale;
	    mXoffset = (mLowerRight.getX() + mUpperLeft.getX())/2 - size.width/mScale/2.0;
	    mYoffset = (mLowerRight.getY() + mUpperLeft.getY())/2 - size.height/mScale/2.0;
    }

    private void updateExtents()
	{
	    boolean first = true;
	    for (Iterator i = mBogies.iterator(); i.hasNext(); )
	    {
	        Bogey b = (Bogey)i.next();
    ...
    private void updateLocations()
    {
        ...
        for (int i = 0; i < bs.length - 1; i++)
        {
            ...
            {	// split up
                ...
                double dTheta = 0;
                for (Iterator k = group.iterator(); k.hasNext(); dTheta += delta)
                {
                    int off = ((Integer)k.next()).intValue();
     ...
     */
    public ArrayList getSelectable()
    {
        return mSelectable;
    }
    /**
     ...
     */
    public void setSelectable(ArrayList selectable)
    {
    ...
    private void paintGrid(Graphics g)
    {
        ...
        for (double dx = step; dx*mScale < size.width/2; dx += step)
        {
            ...
            g.drawLine(size.width/2 + ox, 0, size.width/2 + ox, size.height);
            g.drawLine(size.width/2 - ox, 0, size.width/2 - ox, size.height);
        }
        for (double dy = step; dy*mScale < size.height/2; dy += step)
        {
            int oy = (int)(dy*mScale);
            g.drawLine(0, size.height/2 + oy, size.width, size.height/2 + oy);
            g.drawLine(0, size.height/2 - oy, size.width, size.height/2 - oy);
        }
    }
    ...
    {
        for (Iterator i = mBogies.iterator(); i.hasNext(); )
            paintBogey(g, (Bogey)i.next());
    }
    
    private void paintBogey(Graphics g, Bogey b)
    {
        Point p = (Point)mBogieLocation.get(b);
        Color c;
    ...
    {
        for (Iterator i = mLinks.iterator(); i.hasNext(); )
            paintLink(g, (BogeyLink)i.next());
    }
    
    private void paintLink(Graphics g, BogeyLink l)
    {
        Point source = (Point)mBogieLocation.get(l.getSource());
        Point target = (Point)mBogieLocation.get(l.getTarget());
    ...
    private Bogey findBogey(Point p, boolean selectableOnly)
    {
        ...
        for (Iterator i = mBogieLocation.keySet().iterator(); i.hasNext(); )
        {
            Bogey b = (Bogey)i.next();
            Point bp = (Point)mBogieLocation.get(b);
            if (bp == null)
                continue;
            if ((dist(p, bp) <= 6) && (selectableOnly || mSelectable.contains(b)))
                return b;
        }
        return null;
    }
