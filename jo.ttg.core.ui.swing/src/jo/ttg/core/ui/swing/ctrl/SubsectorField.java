...
public class SubsectorField extends JComponent
{
	...
	protected void doProcessMouseEvent(MouseEvent e)
	{
		...
		SubSectorBean sub = mScheme.getGeneratorSubSector().generateSubSector(o);
		switch (e.getID())
		{
			...
			case MouseEvent.MOUSE_CLICKED:
				if ((e.getClickCount() == 1) && (o != null))
				{
					...
					{
						setSelected(o);
						fireTTGActionEvent(new TTGActionEvent(this, TTGActionEvent.SELECTED, sub.getURI(), sub));				
					}
				}
				else if ((e.getClickCount() == 2) && (sub != null))
				{
					if (e.getButton() == MouseEvent.BUTTON1)
						fireTTGActionEvent(new TTGActionEvent(this, TTGActionEvent.ACTIVATED, sub.getURI(), sub));				
					else
						fireTTGActionEvent(new TTGActionEvent(this, TTGActionEvent.DEACTIVATED, sub.getURI(), sub));				
				}
				break;
			...
			case MouseEvent.MOUSE_MOVED:
				if (sub != null)
				{
					...
					{
						mHoverURI = uri;
						SectorBean sec = mScheme.getGeneratorSector().generateSector(sub.getUpperBound());
						setToolTipText(sec.getName()+" / "+sub.getName());
						fireTTGActionEvent(new TTGActionEvent(this, TTGActionEvent.HOVER, sub.getURI(), sub));				
					}
				}
				else
					mHoverURI = null;
...
//				break;
	...
	public void paint(Graphics g)
	{
		...
		for (int x = 0; x < size.width; x++)
			...
			{
				Rectangle bounds = new Rectangle(x*SUB_W, y*SUB_H, SUB_W, SUB_H);
		...
		if (mSelected != null)
		{
			...
			g.drawRect((int)(mSelected.getX() - mOrigin.getX())*3, 
				(int)(mSelected.getY() - mOrigin.getY())*3, 
				SUB_W, SUB_H);
		}
	}
	...
	{
		OrdBean o = new OrdBean(mOrigin.getX() + x*8, mOrigin.getY() + y*10, mOrigin.getZ());
		SubSectorBean sub = mScheme.getGeneratorSubSector().generateSubSector(o);
