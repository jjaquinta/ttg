...
import ttg.adv.view.dlg.ShipInfoDlg;
...
public class ShipEditPanel extends JPanel
{
    ...
    private JComboBox<String>    mTypePrimary;
    private JComboBox<String>    mTypeQualifier;
    private JTextArea            mNotes;
    private JComboBox<String>    mSection;
    private JList<ShipComponent> mComponentList;
    ...
    private void initInstantiate()
    {
        ...
        if (mShip.getType().length() > 0)
            mTypePrimary.setSelectedItem(ShipFormatLogic
                    .findPrimaryNameFromCode(mShip.getType().substring(0, 1)));
        else
            mTypePrimary.setSelectedIndex(0);
        mTypeQualifier = new JComboBox<>(ShipFormatLogic.getQualifierNames());
        if (mShip.getType().length() > 1)
            mTypeQualifier
                    .setSelectedItem(ShipFormatLogic.findQualifierNameFromCode(
                            mShip.getType().substring(1, 2)));
        else
            mTypeQualifier.setSelectedIndex(0);
        mNotes = new JTextArea(mShip.getNotes());
        mNotes.setLineWrap(true);
        String[] choice = new String[ShipComponent.mSectionDescriptions.length
                + 1];
        choice[0] = "All Sections";
        System.arraycopy(ShipComponent.mSectionDescriptions, 0, choice, 1,
                ShipComponent.mSectionDescriptions.length);
    ...
    private void initLayout()
    {
        ...
        compView.addTab("Table",
                new JScrollPane(mComponentTable,
    ...
    private void initLink()
    {
        ...
        ListenerUtils.change(mComponentList,
                (ev) -> doSelect(mComponentList.getSelectedValue()));
    ...
    private void doStats()
    {
        ...
        mShip.setType(ShipFormatLogic
                .findCodeFromName(mTypePrimary.getSelectedItem().toString()
                        + " " + mTypeQualifier.getSelectedItem().toString()));
    ...
    protected void doEdit()
    {
        ...
        if (mParent instanceof JFrame)
            dlg = new CompEditDlg((JFrame)mParent, (ShipComponent)sel, mStats,
                    mTechLevel);
        else
            dlg = new CompEditDlg((JDialog)mParent, (ShipComponent)sel, mStats,
                    mTechLevel);
        dlg.setVisible(true);
        doStats();
    }
    ...
    {
        List<ShipComponent> compChoice = new ArrayList<>();
        compChoice.addAll(
                ComponentChoiceLogic.getTechLevelComponents(mTechLevel));
        ...
        try
        {
            ...
            {
                ShipComponent comp = (ShipComponent)((ShipComponent)obj)
                        .clone();
                if (mTechLevel > 0)
            ...
            {
                ShipBlockBean block = (ShipBlockBean)((ShipBlockBean)obj)
                        .clone();
    ...
    protected void doReset()
    {
        ...
        for (ShipComponent o : mOriginalShip.getComponents())
        {
            try
            {
                if (o instanceof ShipBlockBean)
                {
                    ShipBlockBean ori = (ShipBlockBean)o;
                    ShipBlockBean clone = (ShipBlockBean)ori.clone();
                    newComps.add(clone);
                }
                else if (o instanceof ShipComponent)
                {
                    ShipComponent ori = (ShipComponent)o;
                    ShipComponent clone = (ShipComponent)ori.clone();
                    newComps.add(clone);
                }
            }
            catch (CloneNotSupportedException e)
        ...
        if (mOriginalShip.getType().length() > 0)
            mTypePrimary
                    .setSelectedItem(ShipFormatLogic.findPrimaryNameFromCode(
                            mOriginalShip.getType().substring(0, 1)));
        else
            mTypePrimary.setSelectedIndex(0);
        if (mOriginalShip.getType().length() > 1)
            mTypeQualifier
                    .setSelectedItem(ShipFormatLogic.findQualifierNameFromCode(
                            mOriginalShip.getType().substring(1, 2)));
        else
            mTypeQualifier.setSelectedIndex(0);
    ...
    {
        ShipInfoDlg dlg = new ShipInfoDlg((JFrame)SwingUtilities.getRoot(this),
                null, mStats);
