...
public class ForSaleFreightDlg extends JDialog
{
	...
	{
	    setTitle(URIUtils.extractName(mGame.getShip().getLocation())+" Freight Yard");
	...
	private void updateLists()
	{
		...
		//System.out.println("ForSale:Updating lists, #worlds="+worlds.size()+".");
			...
			for (Iterator j = thereSys.getSystemRoot().getAllSatelitesIterator(); j.hasNext(); )
	        {
	            ...
	            if (thereObj instanceof BodySpecialAdvBean)
	            {
	                ...
	                if (here.getURI().equals(there.getURI()))
	                    continue;
	    			List<CargoBean> freights = ForSaleLogic.genFreightForSale(mGame, here, there, date);
	        		if (freights.size() == 0)
	        		    continue;
	    			String name = there.getName();
	        		if (name.indexOf(there.getSystem().getName()) < 0)
	        		    name += " ("+there.getSystem().getName()+" System)";
	        		name += ", "+FormatUtils.sTons(CargoLogic.totalTons(freights))+"";
    ...
    protected void doNewDestination()
    {
        ...
        if (name.equals("Show All"))
        {
            ...
            for (Iterator i = mFreightMap.values().iterator(); i.hasNext(); )
            {
        ...
        {
            freights = (ArrayList)mFreightMap.get(name);
        }
	...
	protected void doBuyFreight()
	{
		...
		for (int i = 0; i < sel.length; i++)
			if (!BuyLogic.buyFreight(mGame, mShip, (AdvCargoBean)freights.get(sel[i])))
			    break;
		updateLists();
		updateCapacity();
	}
	...
	protected void doTableClick()
	{
	    ...
	    if (row < 0)
	        return;
	    AdvCargoBean cargo = (AdvCargoBean)mFreightModel.getCargos().get(mFreightSorter.mapRow(row));
