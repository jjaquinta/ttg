 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class ImpGenSystemEx extends ImpGenSystem
{
    
    public ImpGenSystemEx(ImpGenScheme scheme)
    {
        super(scheme);
    }
    ...
    public SystemBean generateSystem(OrdBean ords)
    {
        ...
        if (sys == null)
            return sys;
        if (!mScheme.mExtendedSystemGeneration)
            return sys;
        int industry = 0;
        int science = 0;
        int tech = 0;
        int law = 0;
        List<BodyBean> giants = new ArrayList<BodyBean>();
        List<BodyBean> life = new ArrayList<BodyBean>();
    ...
    private static ImpBodySpecialBean addSpecial(BodyBean body, int subType, Bean specialInfo, String name, int orbit, RandBean rnd, int demand, int production)
    {
        if (body instanceof BodyToidsBean)
            orbit = 0;
        ImpBodySpecialBean special = new ImpBodySpecialBean();
        name = StringUtils.substitute(name, "%n", body.getName());
        special.setOID(RandLogic.rand(rnd));
    ...
    private void addPorts(BodyBean b, PopulatedStatsBean stats, RandBean rnd, int demand, int production)
    {
        ...
        if ((port >= 'A' && (port <= 'E')))
        {	// proper starport
            addPort(b, port, "%n System %p", (tl > 10) ? RandLogic.D(rnd, 3) : 0, rnd, demand, production, stats);
            ...
            while ((pop >= 7) && (port <= 'C'))
            {
                ...
                addPort(b, port - 'A' + 'F', name+" %p", (tl > 10) ? RandLogic.D(rnd, 3) : 0, rnd, demand, production, stats);
                pop -= 2;
                port++;
            }
        }
        ...
        {	// space port
            addPort(b, port, "%n %p", (tl > 10) ? RandLogic.D(rnd, 3) : 0, rnd, demand, production, stats);
        }
    }
    
    private BodySpecialBean addPort(BodyBean body, int type, String name, int orbit, RandBean rnd, int demand, int production, PopulatedStatsBean stats)
    {
        String pName;
        int subType;
        if ((type >= 'A') && (type <= 'E'))
        ...
        ImpBodySpecialBean port = addSpecial(body, subType, p, name, orbit, rnd, (int)(demand*mod), (int)(production*mod));
    ...
    {
        UPPPorBean port = new UPPPorBean();
        port.setValue('F');
        if (b.getPrimary() instanceof BodyGiantBean)
            addSpecial(b.getPrimary(), BodySpecialBean.ST_NAVYBASE, port, "Fort "+b.getName(), 90, rnd, demand, production);
        else
            addSpecial(b, BodySpecialBean.ST_NAVYBASE, port, "Fort %n", 90, rnd, demand*2, production*2);
    }
    ...
    private void addScoutBase(BodyBean b, PopulatedStatsBean stats, RandBean rnd, int demand, int production, int tech, int law)
    {
        ...
        for (int i = children.length - 1; i >= 0; i--)
            ...
            {
                addSpecial(children[i], BodySpecialBean.ST_SCOUTBASE, port, b.getName()+" Scout Base", RandLogic.D(rnd, 10), rnd, 0, 0);
                return;
            }
        ImpBodySpecialBean scout = addSpecial(b, BodySpecialBean.ST_SCOUTBASE, port, "%n Scout Base", RandLogic.D(rnd, 10), rnd, demand, production);
    ...
    private void addRefinery(SystemBean sys, List<BodyBean> giants, RandBean rnd, int tech, int law)
    {
        ...
        UPPPorBean port = new UPPPorBean();
        port.setValue('G');
        ImpBodySpecialBean refinery = addSpecial(parent, BodySpecialBean.ST_REFINERY, port, name+" Refinery", RandLogic.D(rnd, 10), rnd, RandLogic.D(rnd, tech/4), RandLogic.D(rnd, tech/4));
    ...
    private void addLab(SystemBean sys, List<BodyBean> life, RandBean rnd, int tech, int law)
    {
        ...
        IGenLanguage lang = getScheme().getGeneratorLanguage();
        String name = lang.generatePlaceName(sys.getOrds(), sys.getOrds(), "Im", rnd);
        ImpBodySpecialBean lab = addSpecial(parent, BodySpecialBean.ST_LABBASE, null, name+" Lab", RandLogic.D(rnd, 10), rnd, RandLogic.D(rnd, tech/4), RandLogic.D(rnd, tech/4));
    ...
    {
        GoodList goods = ImpGenScheme.getGoods(phylum, type, tech, law);
        GoodBean good = ImpGenScheme.findGood(goods, rnd);
        return good;
    }
