...
//System.out.println(" Val: "+args[0]);                        
    ...
    public static Object get(Object bean, String val)
    {
        ...
        if (val.endsWith("()"))
            return getMethod(bean, val.substring(0, val.length() - 2));
        if (val.startsWith("[") && val.endsWith("]"))
            return getIndex(bean, IntegerUtils.parseInt(val.substring(1, val.length() - 1)));
    ...
    public static Object getIndex(Object bean, int idx)
    {
        if (bean == null)
            return null;
        try
        {
            if (bean instanceof Collection<?>)
                return ArrayUtils.get((Collection<?>)bean, idx);
    ...
    public static Object getMethod(Object bean, String val)
    {
        try
        {
            if (bean == null)
            ...
            if (m == null)
            {
                for (Method mm : bean.getClass().getMethods())
                    if ((mm.getName().equalsIgnoreCase(val) || mm.getName().equalsIgnoreCase("get"+val))
                ...
                if (m == null)
                {
                    System.err.println("No method '"+val+"' found on "+bean.getClass().getName());
                    return null;
                }
            }
